...
const centerColumnFullscreenClasses =
  "flex-1 h-full flex flex-col overflow-hidden border-l border-r border-[#e7eaf3]";
const chatColumnForComparisonViewClasses =
  "flex-1 h-full flex flex-col overflow-hidden border-r border-[#e7eaf3]";
...
function App() {
  ...
  // --- MODIFIED useEffect for loading/clearing active data ---
  const prevSelectedRepoFilenameRef = useRef<string | null | undefined>(
    undefined
  ); // Initialize to undefined to detect first run
  const prevFileContentVersionRef = useRef<number | undefined>(undefined); // Initialize to undefined

  useEffect(() => {
    const filenameChanged =
      selectedRepoFilename !== prevSelectedRepoFilenameRef.current;
    const versionChanged =
      fileContentVersion !== prevFileContentVersionRef.current;

    // Only act if the filename or version has actually changed.
    // This prevents re-running due to changes in function identities from contexts if the core data hasn't changed.
    if (filenameChanged || versionChanged) {
      // console.log('[App.tsx Effect] Conditions met (filenameChanged || versionChanged). filename:', selectedRepoFilename, 'version:', fileContentVersion);
      if (selectedRepoFilename) {
        ...
        // console.log('[App.tsx Effect] selectedRepoFilename is null, calling clearActiveData.');
        clearActiveData();
      }
    }
    // Update refs for the next render AFTER the logic has run
    prevSelectedRepoFilenameRef.current = selectedRepoFilename;
    prevFileContentVersionRef.current = fileContentVersion;
  }, [
    selectedRepoFilename,
    fileContentVersion,
    loadAndParseDataFromServerFile,
  ...
  const displayAttachedFileNameForInputArea = useMemo(() => {
    if (promptSelectedFilePaths.length > 0 && parsedRepomixData) {
      ...
      if (
        allFilesCount > 0 &&
        promptSelectedFilePaths.length === allFilesCount
      ) {
    ...
    if (activeRepomixSource?.filename) {
      const typePrefix =
        activeRepomixSource.type === "manual"
          ? "Manually Attached: "
          : "Active: ";
  ...
  if (
    authIsLoading ||
    (session && apiKey.apiKeyStatusLoading && apiKey.userHasGeminiKey === null)
  ) {
    return (
      <div className="flex justify-center items-center h-screen text-center text-lg">
        Loading application essentials...
      </div>
    );
  }
  if (!session || !user) {
  ...
  const mainContentWrapperHeightClass = view.isFullScreenView
    ? fullscreenMainContentHeight
    : mainContentWrapperHeight;
        ...
        centralPanelComponent={
          <CentralInteractionPanel
            ...
            onFileAttach={(file: File) =>
              loadAndParseDataFromManualFile(file, file.name)
            }
