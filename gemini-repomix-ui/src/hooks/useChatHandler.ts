  ...
  const submitPrompt = useCallback(
    async (prompt: string) => {
      ...
      if (userHasGeminiKey === null && !apiKeyStatusLoading) {
        setChatError(
          "Could not verify API key status. Please try again or set your key."
        );
        openApiKeyModal();
        return;
      }

      const currentAttachedFileName =
        parsedRepomixData && selectedRepoFile ? selectedRepoFile : null;

      if (
        !prompt.trim() &&
        !parsedRepomixData &&
        promptSelectedFilePaths.length === 0
      ) {
        setChatError(
          "Please type a prompt, attach a file, or select files from the tree."
        );
        return;
      }
      ...
      if (
        promptSelectedFilePaths.length > 0 &&
        parsedRepomixData?.fileContents
      ) {
        const selectedFilesContentParts: string[] = [];
        promptSelectedFilePaths.forEach((path) => {
          const content = parsedRepomixData.fileContents[path];
          if (content) {
            selectedFilesContentParts.push(
              `<file path="${path}">\n${content}\n</file>`
            );
          }
        });
      ...
      combinedPromptForApi += prompt
        ? `User Prompt: ${prompt}`
        : contextDescriptor
          ? `User Prompt: Please analyze the ${contextDescriptor}.`
          : `User Prompt: Please analyze.`;

      const userDisplayPrompt =
        prompt || contextDescriptor || "Analyze request";
      const newUserMessage: ChatMessage = {
        role: "user",
        parts: [{ text: userDisplayPrompt }],
      };
