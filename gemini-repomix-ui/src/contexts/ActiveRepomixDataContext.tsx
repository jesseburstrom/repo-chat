...
type ActiveRepomixDataContextType = ActiveRepomixDataState &
  ActiveRepomixDataActions;
const ActiveRepomixDataContext = createContext<
  ActiveRepomixDataContextType | undefined
>(undefined);
...
export const ActiveRepomixDataProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const setAttachmentStatusWithTimeout = useCallback(
    (message: string | null, isError: boolean = false, timeout = 3000) => {
      if (attachmentStatusTimer.current)
        clearTimeout(attachmentStatusTimer.current);
      setState((prev) => ({
        ...prev,
  ...
  const processParsedData = useCallback(
    (
      parsedData: ParsedRepomixData | null,
      filename: string,
      type: "server" | "manual"
    ) => {
      ...
      setAttachmentStatusWithTimeout(
        parsedData
          ...
          : `Attached ${filename} (non-standard format or parse error)`
      );
      return !!parsedData;
    },
    [setAttachmentStatusWithTimeout]
  ...
  const loadAndParseDataFromServerFile = useCallback(
    async (filename: string): Promise<boolean> => {
      if (!session) {
        ...
        setState((prev) => ({
          ...prev,
          dataError: "Authentication required to load server file.",
        }));
        return false;
      }
      clearActiveData();
  ...
  const loadAndParseDataFromManualFile = useCallback(
    async (file: File, filename: string): Promise<boolean> => {
      clearActiveData();
      setState((prev) => ({ ...prev, isLoadingData: true, dataError: null }));
      ...
      return new Promise<boolean>((resolve) => {
        ...
        reader.onerror = (err) => {
          setState((prev) => ({
            ...prev,
            isLoadingData: false,
            dataError: `Error reading file: ${String(err)}`,
          }));
          setAttachmentStatusWithTimeout(
    ...
    [clearActiveData, setAttachmentStatusWithTimeout, processParsedData]
  );

  const selectFileForViewing = useCallback(
    (filePath: string) => {
      if (
        !state.parsedRepomixData ||
        !filePath ||
        !state.parsedRepomixData.fileContents
      ) {
        setState((prev) => ({
          ...prev,
          selectedFilePathForView: null,
      ...
      setState((prev) => ({
        ...
        selectedFileContentForView:
          content !== undefined
            ? content
            : `// Error: Content for ${filePath} not found.`,
      }));
    },
    [state.parsedRepomixData]
  );
  ...
  const selectAllPromptFiles = useCallback(() => {
    if (state.allFilePathsInCurrentRepomix.length > 0) {
      setState((prev) => ({
        ...prev,
        promptSelectedFilePaths: [...prev.allFilePathsInCurrentRepomix],
      }));
    }
...
export const useActiveRepomixData = () => {
  ...
  if (!context)
    throw new Error(
      "useActiveRepomixData must be used within an ActiveRepomixDataProvider"
    );
  return context;
};
